...
- **[Reverse proxy recipes](/docs/sheets/reverse-proxy/)** — nginx, Caddy,
  Traefik. The WebSocket upgrade header is the most-common
  gotcha; recipes include the minimum config + commentary.
- **[TLS + custom domain](/docs/sheets/tls/)** — Let's Encrypt with each
  proxy, DNS pointers, the `CASUAL_PUBLIC_ORIGIN` setting.
- **[CORS](/docs/sheets/cors/)** — when you need it, when you don't, the
  `CASUAL_CORS_ORIGINS` env var, the most common mistake.
- **[Scaling](/docs/sheets/scaling/)** — single-process limits today, Redis
  persistence, horizontal scale-out caveats labelled as v0.2.
- **[Backups](/docs/sheets/backups/)** — per-backend recipes (S3 versioning,
