...
- **[Branding](/docs/sheets/branding/)** — app name, accent colour, logo.
- **[Storage](/docs/sheets/storage/)** — workbook persistence backend
  selection + per-backend creds.
- **[Networking](/docs/sheets/networking/)** — public origin, CORS, trust
  proxy, HSTS.
- **[Base path](/docs/sheets/base-path/)** — reverse-proxy sub-path mount.
- **[Room limits](/docs/sheets/room-limits/)** — max rooms / file size /
  TTL / users per room.
- **[Auth providers](/docs/sheets/auth/)** — JWT (live), OIDC + SAML
  (stubs for v0.2).
- **[Webhooks](/docs/sheets/webhooks/)** — event-driven HTTP POSTs with
  HMAC signing.
...
See [`auth.md`](/docs/sheets/auth/) for the full claim model + role
permissions matrix.
